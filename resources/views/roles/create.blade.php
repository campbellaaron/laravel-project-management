...
@section('content')
    ...
    <div class="bg-gray-200 dark:bg-gray-900">
        ...
        @endif
        <form action="{{ route('roles.store') }}" method="POST" class="bg-white dark:bg-slate-700 shadow-md rounded px-8 pt-6 pb-8 mb-4 max-w-3xl mx-auto">
            @csrf
            <div class="mb-4">
                <label for="role_name" class="block text-sm font-medium text-gray-700 dark:text-indigo-50">Role Name</label>
                <input type="text" name="role_name" id="role_name" class="mt-1 p-2 block w-full rounded-md border outline-none border-gray-300 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500" required>
                @error('role_name')
                                ...
                                @foreach ($role_permissions as $permission)
                                    <tr>
                                        ...
                                        <td class="styled-checkbox flex justify-center p-5">
                                            <input type="checkbox" name="permission[]" value="{{ $permission->name }}" id="permission_{{ $permission->name }}" class="peer h-5 w-5 cursor-pointer transition-all appearance-none rounded shadow hover:shadow-md border border-slate-300 checked:bg-slate-800 checked:border-slate-800">
            ...
            <button type="submit" class="focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Create Role</button>
        </form>
    </div>
@endsection
